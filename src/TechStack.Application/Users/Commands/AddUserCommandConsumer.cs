...
public class AddUserCommandConsumer(IApplicationDbContext applicationDbContext, UserMapper mapper) : IConsumer<AddUserCommand>
{
    private readonly IApplicationDbContext applicationDbContext = applicationDbContext;
    private readonly UserMapper mapper = mapper;

    public async Task Consume(ConsumeContext<AddUserCommand> context)
    {
        var user = mapper.AddUserCommandToUser(context.Message);
